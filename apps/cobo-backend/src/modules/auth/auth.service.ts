...
export class AuthService {
	...
	async validateUser(phonenumber: string, pass: string): Promise<any> {
		...
		const compareResult = await this.bcryptService.compareWithHash(pass, user.password);
	...
	async validateUserRefreshToken(user: User, refreshToken: string) {
		if (!user.refreshToken) {
			...
			if (isMatch) {
				try {
					const token = await this.jwtService.verifyAsync<RefreshToken>(refreshToken, {
	...
	async refresh(userId: string, refreshToken: string) {
		...
		const refreshTokenMatches = this.bcryptService.compareWithHash(user.refreshToken, refreshToken);
